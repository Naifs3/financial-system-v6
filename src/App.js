...
export default function App() {
  ...
  if (!isLoggedIn) {
    return (
      <div className={`min-h-screen ${bg} flex items-center justify-center p-4`} style={{ fontFamily: currentFont.value }}>
        <link href={currentFont.url} rel="stylesheet" />
        <div className={`${darkMode ? 'bg-gradient-to-br from-gray-800 to-gray-700' : 'bg-white'} rounded-3xl p-8 max-w-md w-full shadow-2xl border ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          <div className="text-center mb-8">
            <div className={`inline-flex items-center justify-center w-20 h-20 rounded-full bg-gradient-to-r ${accent.gradient} mb-4 shadow-lg`}>
              <Shield className="w-10 h-10 text-white" />
            </div>
            <h1 className={`text-3xl font-bold ${txt} mb-2`}>تسجيل الدخول</h1>
            <p className={`${txtSm}`}>أدخل بياناتك للوصول إلى النظام</p>
          </div>

          <div className="space-y-4">
            <div>
              <label className={`block text-sm font-medium ${txt} mb-2 flex items-center gap-2`}>
              ...
              <input
                ...
                onKeyPress={(e) => {
                  if (e.key === 'Enter') {
                    const user = users.find(u => u.username === loginUsername && u.password === loginPassword && u.active !== false);
                    if (user) {
                      setCurrentUser(user);
                      setIsLoggedIn(true);
                      localStorage.setItem('currentUser', JSON.stringify(user));
                      localStorage.setItem('isLoggedIn', 'true');
                      setLoginLog([...loginLog, { user: user.username, timestamp: new Date().toISOString(), action: 'login' }]);
                    } else {
                      alert('بيانات الدخول غير صحيحة أو الحساب غير نشط');
                    }
                  }
                }}
                className={`w-full p-3 rounded-xl border ${darkMode ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400' : 'bg-white border-gray-300 text-gray-900'} focus:ring-2 focus:ring-${accentIndex === 0 ? 'blue' : accentIndex === 1 ? 'purple' : 'emerald'}-500 transition-all`}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className={`absolute left-3 top-1/2 -translate-y-1/2 ${txtSm} hover:${txt} transition-colors`}
                >
            ...
            <button
              ...
              className={`w-full py-3 bg-gradient-to-r ${accent.gradient} text-white rounded-xl font-medium hover:opacity-90 transition-all flex items-center justify-center gap-2 mt-6 shadow-lg`}
            >
          ...
          <div className={`mt-6 text-center ${txtSm} text-xs space-y-1`}>
            <p>إصدار {APP_VERSION}</p>
            <p className="text-[10px] opacity-60">ركائز الأولى للتعمير</p>
          </div>
        </div>
      </div>
    );
